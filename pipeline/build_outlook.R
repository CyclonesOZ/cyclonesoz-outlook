...
# Points are fetched in batches (multi-location requests) for speed.
# ---------------------------------------------------------------------------
suppressMessages({
...
BATCH  <- 8            # grid points per Open-Meteo request
...
om_url <- function(lats, lons){                # lats/lons are comma-joined strings
  lv <- paste0(c(
  ...
  sprintf(paste0("https://api.open-meteo.com/v1/forecast?latitude=%s&longitude=%s",
    "&hourly=%s,%s&forecast_days=4&timezone=auto&wind_speed_unit=kn&cell_selection=nearest"),
    lats, lons, sfc, lv)
}

# fetch a batch of points; returns a list of per-location results (or NULL)
fetch_batch <- function(pts){
  lats <- paste(sprintf("%.3f", pts[,1]), collapse=",")
  lons <- paste(sprintf("%.3f", pts[,2]), collapse=",")
  url  <- om_url(lats, lons)
  for (a in 1:4){
    res <- tryCatch(fromJSON(url, simplifyDataFrame=FALSE), error=function(e) NULL)
    if (!is.null(res)){
      if (!is.null(res[["hourly"]])) return(list(res))  # single-location response
      return(res)                                       # list of locations
    }
    Sys.sleep(1.5*a)
  }
  NULL
...
cat(sprintf("Processing %d grid points in batches of %d...\n", nrow(GRID), BATCH))
points <- vector("list", nrow(GRID)); day_labels <- NULL; ok <- 0

start <- 1
while (start <= nrow(GRID)){
  fin   <- min(start + BATCH - 1, nrow(GRID))
  batch <- GRID[start:fin, , drop=FALSE]
  res   <- fetch_batch(batch)
  for (j in seq_len(nrow(batch))){
    k <- start + j - 1
    r <- if (is.null(res) || j > length(res)) NULL else res[[j]]
    if (is.null(r) || is.null(r$hourly)){ points[[k]] <- NULL; next }
    ...
    points[[k]] <- list(lat=GRID[k,1], lon=GRID[k,2], d=dres)
    ok <- ok + 1
  }
  cat(sprintf("  %d/%d\n", fin, nrow(GRID)))
  start <- fin + 1
